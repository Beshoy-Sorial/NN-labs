...
class ClassificationNeuralNet():
    def __init__(self, structure, activation='relu', random_state=42, batch_size=128, epochs=20, α=0.02, eval_train=False):
        ...
        self.h, self.hࠤ = (σ, σࠤ) if self.activation=='sigmoid' else (relu, reluࠤ)
        ...
        self.Wₙ = [np.random.randn(n_l, n_l_prev) / (np.sqrt(n_l_prev) if activation=='sigmoid' else np.sqrt(2/n_l_prev)) 
                                                                  for n_l, n_l_prev in zip(structure[1:], structure[:-1])]                             
    ...
    def feedforward(self, x, store_outputs=False):
        ...
        for l, (b, W) in enumerate(zip(self.Bₙ, self.Wₙ)):          # loop on each layer (after input layer)
            ...
            a = self.h(z) if l < self.num_layers-1 else self.g(z)
    ...
    def SGD(self, x_batch, y_batch, α):
        ...
        self.Bₙ = [self.Bₙ[l] - α/self.batch_size * მJⳆმBₙ[l] for l in range(self.num_layers)]      # gift
        self.Wₙ = [self.Wₙ[l] - α/self.batch_size * მJⳆმWₙ[l] for l in range(self.num_layers)]
